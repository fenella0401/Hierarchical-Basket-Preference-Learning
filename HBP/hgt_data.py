...
'''refers to https://arxiv.org/abs/2003.01332'''
...
def preprocess(node, links, n_hid, attributed): 
    ...
    #ntypes所有的node节点{节点id，节点type}
    ntypes, insize = {}, n_hid
    if attributed=='True': nfeatures = {}

    with open('/devdata/fenella/KGs/OpenKE/benchmarks/MFP/ent_embeddings', 'rb') as f:
        ent_emb = pkl.load(f)
    ...
    with open(node, 'r') as file:
        for line in file:
            ...
            if attributed=='False': nid, _, ntype = line[:-1].split('\t')
            elif attributed=='True': 
                nid, _, ntype = line[:-1].split('\t')
                nfeatures[int(nid)] = ent_emb[count]
                count += 1
            ntypes[nid] = ntype
